...
function Page() {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete('/admin/recruitments/delete/' + id);
  ...
  const handleEdit = (id) => {
    router.push(`/admin/recruitments/edit/${id}`);
  };
  ...
  return (
    <div className="mt-16 ml-6">
      <div className="h-screen w-full mr-6 ">
        <h1 className="w-full text-center font-extrabold text-3xl my-5 ">Recruitments</h1>
        <div className="mt-10 flex justify-center">
          <button
            onClick={() => router.push('/admin/recruitments/add')}
            type="submit"
